...
```
legendary launch <App Name> [options] [-- <game args>]
```

Any arguments that follow the known Legendary flags are passed through directly to the game process. The `<App Name>` accepts the exact app name, a partial title match, or a configured alias.
...
<Note>
  Any flags not recognized by Legendary are passed through to the game process. To avoid ambiguity, place game-specific arguments after all Legendary flags.
</Note>
